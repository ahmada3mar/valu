...
{    protected $connection = 'mongodb';
    ...
    {
        $is_role_edite = empty($model->getDirty()) ;
        return new static([
            'batch_id' => (string) Str::orderedUuid(),
            'user_id' => $user->getAuthIdentifier(),
            'name' => $is_role_edite ? 'Update Permessions' : 'Update',
            'actionable_type' => $model->getMorphClass(),
            'actionable_id' => $model->getKey(),
            'target_type' => $model->getMorphClass(),
            'target_id' => $model->getKey(),
            ...
            'model_id' => $model->getKey(),
            'fields' => '',
            'original' => $is_role_edite ? null : array_intersect_key($model->getRawOriginal(), $model->getDirty()) ,
            'changes' => $is_role_edite ? null : $model->getDirty(),
            'status' => 'finished',
            'exception' => '',
        ]);
    }
